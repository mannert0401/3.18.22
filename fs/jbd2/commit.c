...
/* [NHJ] UFS: The last block of journal metadata  */
...
/* 
 * [NHJ] UFS: IO end handlers for io_bufs
...
/* [NHJ] UFS: IO end handler for commit record */
...
static int journal_submit_inode_data_buffers(struct address_space *mapping)
{
	...
	struct writeback_control wbc = {
		.sync_mode =  WB_ORDERED_ALL, /* [NHJ] UFS: WB_SYNC_ALL,*/
...
void jbd2_journal_barrier_commit_transaction(journal_t *journal)
{
        ...
        int temp_conflict_count;
#endif
        int is_booting_disk = 0;

        if (journal->j_dev->bd_dev == 8388609 &&
                journal->j_dev->bd_dev ==8388851) {
                is_booting_disk = 1;
        }
...
#ifdef DELAY_ON_COMMIT_WITH_LOOP
commitloop:
#endif
        write_lock(&journal->j_state_lock);
        commit_transaction->t_state = T_LOCKED;
#ifdef DEBUG_COMMIT
        commit_transaction->t_start_commit = ktime_get();
#endif
        ...
        stats.run.rs_running = jbd2_time_diff(commit_transaction->t_start,
                                              stats.run.rs_locked);
#ifdef INFO_USEC
        commit_transaction->t_locked_time = ktime_get();
#endif
       ...
       while (atomic_read(&commit_transaction->t_updates)) {
...
#ifdef _JBD2_BARRIER_COMMITTING_TX
        temp_conflict_count = atomic_read(&commit_transaction->t_conflict_count);
#endif
         ...
         if ((jbd2_psp_mode & JBD2_BARRIER_CONFLICT_COUNT) &&
                 ...
                 if (jbd2_psp_mode & DEBUG_CONFLICT_COUNT) {
                         printk("[DEBUG_CONFLICT_COUNT:count-loop] t_conflict_count : %d\n", atomic_read(&commit_transaction->t_conflict_count));
                 }
                 if (atomic_read(&commit_transaction->t_conflict_count)) {
                         J_ASSERT(0 <= atomic_read(&commit_transaction->t_conflict_count));
         ...
         } else if (jbd2_psp_mode & JBD2_BARRIER_CONFLICT_COUNT) {
                 
                 if (jbd2_psp_mode & DEBUG_CONFLICT_COUNT) {
                         printk("[DEBUG_CONFLICT_COUNT:count] t_conflict_count : %d\n", atomic_read(&commit_transaction->t_conflict_count));
                 }
         ...
         } else if (jbd2_psp_mode & DELAY_ON_COMMIT_WITH_LOOP) {
                 ...
                 if (jbd2_psp_mode & DEBUG_CONFLICT_COUNT) {
                        ...
                        /* [NHJ] UFS : Delayed Commit */
                        spin_lock(&journal->j_list_lock);
                         list_for_each(iter, &commit_transaction->t_jh_wait_list) {
                                 counter++;
                         }
                         spin_unlock(&journal->j_list_lock);
                         printk("[DEBUG_CONFLICT_COUNT:count] t_conflict_count : %d\n", counter);
		...
		/* [NHJ] UFS : Delayed commit*/
                spin_lock(&journal->j_list_lock);
                 while(!list_empty(&commit_transaction->t_jh_wait_list)){
                         ...
                         wake_up(&journal->j_wait_transaction_locked);
                         wait_event(journal->j_wait_done_cpsetup,
                                         list_empty(&commit_transaction->t_jh_wait_list));
                         goto commitloop;
                 }
                 spin_unlock(&journal->j_list_lock);
         } else {
        ...
        while(!list_empty(&commit_transaction->t_jh_wait_list)){
         ...
         write_lock(&journal->j_state_lock);
#endif // DELAY_ON_COMMIT_WITH_LOOP && JBD2_BARRIER_CONFLICT_COUNT
       ...
       while (commit_transaction->t_reserved_list) {
               ...
               if (jh->b_committed_data) {
                       struct buffer_head *bh = jh2bh(jh);
                        ...
                        if(jh->c_shadow_head) {
                                struct shadow_head * sh = jh->c_shadow_head;
                                list_add_tail(&sh->b_sh_pool_list, journal->j_shadow_head_pool);
                                jh->c_shadow_head = NULL;
                        } else {
                                jbd2_free(jh->b_committed_data, bh->b_size);
                        }
#else
        ...
        journal->j_running_transaction = NULL;
#ifdef DEBUG_COMMIT
        commit_transaction->t_end_commit = ktime_get();
        commit_transaction->t_intv[0] = ktime_sub(commit_transaction->t_end_commit, 
                                                commit_transaction->t_start_commit);
#endif
#ifdef INFO_USEC
        commit_transaction->t_dispatch_time = ktime_get();
#endif
 
#ifdef JBD2_BARRIER_COMMITTING_TX
        atomic_inc(&journal->j_committing_transactions);
        //insert_committing_node(journal, journal->j_committing_transactions, temp_conflict_count);
        /*
        if (!is_booting_disk)
                printk ("[%s] con_tx %d\n",
                                __func__,
                                atomic_read(&journal->j_committing_transactions));
        */
#endif
        start_time = ktime_get();
        commit_transaction->t_log_start = journal->j_head;
...
start_journal_io:
...
#ifdef DEBUG_COMMIT
        commit_transaction->t_end_commit = ktime_get();
        commit_transaction->t_intv[1] = ktime_sub(commit_transaction->t_end_commit, 
                                                commit_transaction->t_start_commit);
#endif
        /* UFS */
        ...
        blk_finish_plug(&plug);
#ifdef INFO_USEC
        commit_transaction->t_dma_time = ktime_get();
#endif
        ...
        */
...
#ifdef INFO_USEC
        commit_transaction->t_flush_time = ktime_get();
#endif
        jbd_debug(3, "JBD2: commit phase 4\n");
        ...
        write_unlock(&journal->j_state_lock);
	/*
	 * [NHJ] UFS: Inserting this transaction to j_checkpoint_transactions is
...
#ifdef DEBUG_COMMIT
        commit_transaction->t_end_commit = ktime_get();
        commit_transaction->t_intv[2] = ktime_sub(commit_transaction->t_end_commit, 
                                                commit_transaction->t_start_commit);
#endif
...
restart_loop:
        ...
        while (commit_transaction->t_forget) {
                 ...
                 */
                 ...
                 if (jh->b_committed_data) {
                         if(jh->c_shadow_head) {
                                 struct shadow_head * sh = jh->c_shadow_head;
                                 list_add_tail(&sh->b_sh_pool_list, sh->journal->j_shadow_head_pool);
                                 jh->c_shadow_head = NULL;
                        } else {
                                jbd2_free(jh->b_committed_data, bh->b_size);
                                }
                         ...
                         if (jh->b_frozen_data) {
                                 if(jh->f_shadow_head) {
                                         J_ASSERT(jh->f_shadow_head->b_frozen == jh->b_frozen_data);
                                         jh->c_shadow_head = jh->f_shadow_head;
                                         jh->f_shadow_head = NULL;
                                 }
                                         jh->b_committed_data = jh->b_frozen_data;
                                         jh->b_frozen_data = NULL;
                                         jh->b_frozen_triggers = NULL;
                        ...
                        if (jh->f_shadow_head) {
                                 struct shadow_head * sh = jh->f_shadow_head;
                                 list_add_tail(&sh->b_sh_pool_list, sh->journal->j_shadow_head_pool);
                                 jh->f_shadow_head = NULL;
                         } else {
                                 jbd2_free(jh->b_frozen_data, bh->b_size);
                ...
                */
                ...
                if (!is_booting_disk && jh->b_next_transaction) {
#ifdef PROACTIVE_SHADOWING_PAGE
                        jh->b_hot |= 1;
#endif
 
#ifdef PAGE_CONFLICT_PROFILE
                        b_blocknr = jh2bh(jh)->b_blocknr;
                        spin_lock(&journal->j_pcl_lock);
                        if (journal->pcl == NULL) {
                                ...
                                insert_conflict_pn(journal->pcl, b_blocknr, 0);
                        }
                        spin_unlock(&journal->j_pcl_lock);
#endif
                }
                JBUFFER_TRACE(jh, "refile or unfile buffer");
                __jbd2_journal_refile_buffer(jh);
        ...
        running_time_u = ktime_to_ns(ktime_sub(commit_transaction->t_locked_time, commit_transaction->t_start_time));
        locked_time_u = ktime_to_ns(ktime_sub(commit_transaction->t_dispatch_time, commit_transaction->t_locked_time));
        dispatch_time_u = ktime_to_ns(ktime_sub(commit_transaction->t_dma_time, commit_transaction->t_dispatch_time));
        dma_time_u = ktime_to_ns(ktime_sub(commit_transaction->t_flush_time, commit_transaction->t_dma_time));
        flush_time_u = ktime_to_ns(ktime_sub(ktime_get(), commit_transaction->t_flush_time));
 
        if (likely(journal->j_average_running))
		journal->j_average_running = (running_time_u + 
				journal->j_average_running*3) / 4;
	else
		journal->j_average_running = running_time_u;

        if (likely(journal->j_average_locked))
                journal->j_average_locked = (locked_time_u +
		                journal->j_average_locked*3) / 4;
        else
                journal->j_average_locked = locked_time_u;

        if (likely(journal->j_average_dispatch))
                journal->j_average_dispatch = (dispatch_time_u +
                                journal->j_average_dispatch*3) / 4;
        else
                journal->j_average_dispatch = dispatch_time_u;

        if (likely(journal->j_average_dma))
                journal->j_average_dma = (dma_time_u +
		                journal->j_average_dma*3) / 4;
        else
                journal->j_average_dma = dma_time_u;

        if (likely(journal->j_average_flush))
                journal->j_average_flush = (flush_time_u +
                                journal->j_average_flush*3) / 4;
        else
                journal->j_average_flush = flush_time_u;
#endif
...
#ifdef DEBUG_COMMIT
        commit_transaction->t_end_commit = ktime_get();
        commit_transaction->t_intv[3] = ktime_sub(commit_transaction->t_end_commit, 
                                                commit_transaction->t_start_commit);
        if (!is_booting_disk)
                printk("[%s] %lld %lld %lld %lld\n",
                        __func__,
                       ktime_to_ns(commit_transaction->t_intv[0]),
                       ktime_to_ns(commit_transaction->t_intv[1]),
                       ktime_to_ns(commit_transaction->t_intv[2]),
                       ktime_to_ns(commit_transaction->t_intv[3]));
#endif
	write_unlock(&journal->j_state_lock);
...
#ifdef JBD2_BARRIER_COMMITTING_TX
        atomic_dec(&journal->j_committing_transactions);
#endif
